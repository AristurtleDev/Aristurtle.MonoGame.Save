...
public class Game1 : Game
{
    ...
    protected override void Update(GameTime gameTime)
    {
        ...
        {
            string path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "Game.Save.bin");
            ...
            SaveFileWriter.ToBin(path, data);
        ...
        {
            string path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "Game.Save.bin");
            byte[] data = SaveFileReader.FromBin(path);
